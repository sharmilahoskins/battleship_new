...
function makeShipHoriz(shipLength) {
  ...
  function pullCoord() {
    ...
    for (i = 0; i < shipLength; i++) {
      holdingArray.push(coordinate1.toString() + (coordinate2+i).toString())
      console.log("holding array " + holdingArray)
    }
    checkCoordHoriz(holdingArray)
  }

  function checkCoordHoriz(holdingArray) {
    holdingArray.forEach(function(element){
      for(i = 0; i<shipArray.length; i++){
        if(element == shipArray[i]){
        console.log("pulling new coord horiz")
        holdingArray.length = 0
        console.log("holding array is clear?" + holdingArray)
        pullCoord()
        } else {
        placeShip() //but only want to do this once(not in the loop)
        }
      }
    })
  }
  ...
  function placeShip() {
    holdingArray.forEach(function(element){
      shipArray.push(element)
      console.log("shiparray" + shipArray)
    })
    coordinate1 = parseInt(holdingArray[0][0]);
    coordinate2 = parseInt(holdingArray [0][1]);
    for (j=0; j<shipLength; j++) {
      board[coordinate1][coordinate2+j]= ship
    }
    $("#"+ coordinate1 + (coordinate2 + i)).addClass("ship" + shipLength)
  }
}
